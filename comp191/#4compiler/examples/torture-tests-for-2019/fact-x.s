...
love:
    ...
    push SOB_NIL_ADDRESS
			mov rax, qword [fvar_tbl + WORD_SIZE*46];get fvar:append
		IS_UNDEF sym46
    push rax
			mov rax, qword [fvar_tbl + WORD_SIZE*48];get fvar:list
		IS_UNDEF sym48
    push rax
			mov rax, qword [fvar_tbl + WORD_SIZE*2];get fvar:cdr
		IS_UNDEF sym2
    ...
    violation_check rax, T_CLOSURE
      REST_APPLIC
    mov qword [fvar_tbl + WORD_SIZE*47], rax;setting fvar:reverse
    mov rax, SOB_VOID_ADDRESS
		call write_sob_if_not_void
    ...
    push rax
		...
		IS_UNDEF sym46
    push rax
			mov rax, qword [fvar_tbl + WORD_SIZE*47];get fvar:reverse
		IS_UNDEF sym47
    push rax
			mov rax, qword [fvar_tbl + WORD_SIZE*10];get fvar:null?
		IS_UNDEF sym10
