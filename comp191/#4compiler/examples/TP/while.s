...
jmp sympass0
  ...
  jmp sympass38
	sym38:
		db 99,111,110,116,114,111,108, 0
	sympass38:
...
love:
    ...
    mov qword [fvar_tbl + WORD_SIZE*40], rax;setting fvar:i
    ...
    push SOB_NIL_ADDRESS
    push qword 0
    GET_BOUND rax,0,1;get bound:dit
    violation_check rax, T_CLOSURE
      REST_APPLIC
		push SOB_NIL_ADDRESS
			GET_BOUND rax,0,2;get bound:dif
    push rax
			GET_BOUND rax,0,1;get bound:dit
    push rax
			GET_BOUND rax,0,0;get bound:pred
    push rax
    
    mov rbx, fvar_tbl + WORD_SIZE*39
       mov rax, qword [rbx];get fvar:while
		IS_UNDEF rbx
    violation_check rax, T_CLOSURE
    TP_SHIFT_FRAME 3
    ...
    ret
		doneclousure1:
    mov qword [fvar_tbl + WORD_SIZE*39], rax;setting fvar:while
    mov rax, SOB_VOID_ADDRESS
		call write_sob_if_not_void
    ...
    mov rax, [const_tbl + 32]
    mov qword [fvar_tbl + WORD_SIZE*38], rax;setting fvar:control
    mov rax, SOB_VOID_ADDRESS
		call write_sob_if_not_void
    ...
    LAMBDA_ARG_CHECK 5
    push SOB_NIL_ADDRESS
			mov rax, PVAR(0);get param:init
    push rax
    push qword 1
    mov rax, PVAR(1);get param:pred
    violation_check rax, T_CLOSURE
      REST_APPLIC
    mov rbx, SOB_FALSE_ADDRESS
    cmp rbx, rax
    ...
    push SOB_NIL_ADDRESS
    push qword 0
    GET_BOUND rax,0,3;get bound:dit
    violation_check rax, T_CLOSURE
      REST_APPLIC
		push SOB_NIL_ADDRESS
			GET_BOUND rax,0,4;get bound:dif
    push rax
			GET_BOUND rax,0,3;get bound:dit
    push rax
			GET_BOUND rax,0,2;get bound:update
    push rax
			GET_BOUND rax,0,1;get bound:pred
    push rax
			push SOB_NIL_ADDRESS
    ...
    mov rbx, fvar_tbl + WORD_SIZE*37
       mov rax, qword [rbx];get fvar:for
		IS_UNDEF rbx
    violation_check rax, T_CLOSURE
    TP_SHIFT_FRAME 5
    REST_APPLICTP;seq ret
    ...
    ret
		doneclousure12:
    mov qword [fvar_tbl + WORD_SIZE*37], rax;setting fvar:for
    mov rax, SOB_VOID_ADDRESS
		call write_sob_if_not_void
    ...
    mov rbx, fvar_tbl + WORD_SIZE*38
       mov rax, qword [rbx];get fvar:control
		IS_UNDEF rbx
    leave
    ret
		doneclousure14:
