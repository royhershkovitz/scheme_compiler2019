...
const_tbl:
  ...
  db 98,111,111,108,101,97,110

	MAKE_LITERAL_STRING (5)
    
  db 102,108,111,97,116
  ...
  db 116,97,110,122,97,110,105,97

	MAKE_LITERAL_SYMBOL (const_tbl+306)
  ...
  db 101,113,117,97,108,63

	MAKE_LITERAL_STRING (1)
    
  db 61

	MAKE_LITERAL_FLOAT (1.1)
	MAKE_LITERAL_STRING (1)
...
love:
    ...
    ret
		doneclousure4:
    mov qword [fvar_tbl + WORD_SIZE*51], rax;setting fvar:list
    mov rax, SOB_VOID_ADDRESS
		call write_sob_if_not_void
    ...
    push SOB_NIL_ADDRESS
		...
		IS_UNDEF sym49
    push rax
			mov rax, qword [fvar_tbl + WORD_SIZE*51];get fvar:list
		IS_UNDEF sym51
    push rax
			mov rax, qword [fvar_tbl + WORD_SIZE*3];get fvar:cdr
		IS_UNDEF sym3
    ...
    violation_check rax, T_CLOSURE
      REST_APPLIC
    mov qword [fvar_tbl + WORD_SIZE*50], rax;setting fvar:reverse
    mov rax, SOB_VOID_ADDRESS
		call write_sob_if_not_void
    ...
    push rax
		...
		IS_UNDEF sym49
    push rax
			mov rax, qword [fvar_tbl + WORD_SIZE*50];get fvar:reverse
		IS_UNDEF sym50
    push rax
			mov rax, qword [fvar_tbl + WORD_SIZE*11];get fvar:null?
		IS_UNDEF sym11
    ...
    push SOB_NIL_ADDRESS
			mov rax, qword [fvar_tbl + WORD_SIZE*35];get fvar:>
		IS_UNDEF sym35
    push rax
    push qword 1
    EXTEND_VEC rax, rbx
    ...
    violation_check rax, T_CLOSURE
      REST_APPLIC
    mov qword [fvar_tbl + WORD_SIZE*40], rax;setting fvar:positive?
    mov rax, SOB_VOID_ADDRESS
		call write_sob_if_not_void
    ...
    mov rax, SOB_VOID_ADDRESS
		...
		push SOB_NIL_ADDRESS
			mov rax, qword [fvar_tbl + WORD_SIZE*37];get fvar:x
		IS_UNDEF sym37
    push rax
    push qword 1
    mov rax, qword [fvar_tbl + WORD_SIZE*2];get fvar:car
    ...
    push rax
    push qword 2
    mov rax, qword [fvar_tbl + WORD_SIZE*6];get fvar:set-cdr!
		IS_UNDEF sym6
    violation_check rax, T_CLOSURE
      REST_APPLIC
		call write_sob_if_not_void
    ...
    push rax
    push qword 1
    mov rax, qword [fvar_tbl + WORD_SIZE*7];get fvar:boolean?
		IS_UNDEF sym7
    violation_check rax, T_CLOSURE
      REST_APPLIC
		call write_sob_if_not_void
    ...
    push rax
    push qword 1
    mov rax, qword [fvar_tbl + WORD_SIZE*27];get fvar:integer->char
		IS_UNDEF sym27
    violation_check rax, T_CLOSURE
      REST_APPLIC
		call write_sob_if_not_void
    ...
    push rax
    push qword 1
    mov rax, qword [fvar_tbl + WORD_SIZE*25];get fvar:symbol->string
		IS_UNDEF sym25
    violation_check rax, T_CLOSURE
      REST_APPLIC
		call write_sob_if_not_void
    ...
    violation_check rax, T_CLOSURE
      REST_APPLIC
		...
		mov rax, const_tbl + 389
		call write_sob_if_not_void

		push SOB_NIL_ADDRESS
			mov rax, const_tbl + 399
    push rax
			mov rax, const_tbl + 101
    push rax
    push qword 2
