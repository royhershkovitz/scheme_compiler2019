...
const_tbl:
  ...
  db 116,104,105,115,32,115,104,111,117,108,100,32,98,101,32,97,110,32,101,114,114,111,114,44,32,98,117,116,32,121,111,117,32,100,111,110,39,116,32,115,117,112,112,111,114,116,32,101,120,99,101,112,116,105,111,110,115
	...
	MAKE_LITERAL_PAIR (const_tbl+178, const_tbl+196)
	MAKE_LITERAL_PAIR (const_tbl+101, const_tbl+213)
	MAKE_LITERAL_STRING (5)
    
  db 113,117,111,116,101

	MAKE_LITERAL_SYMBOL (const_tbl+247)
	MAKE_LITERAL_PAIR (const_tbl+1, const_tbl+1)
	MAKE_LITERAL_PAIR (const_tbl+261, const_tbl+270)
	MAKE_LITERAL_PAIR (const_tbl+287, const_tbl+1)
	MAKE_LITERAL_PAIR (const_tbl+187, const_tbl+304)
	MAKE_LITERAL_PAIR (const_tbl+178, const_tbl+321)
	MAKE_LITERAL_PAIR (const_tbl+101, const_tbl+338)
	MAKE_LITERAL_INT (4)
	MAKE_LITERAL_INT (5)
	MAKE_LITERAL_PAIR (const_tbl+381, const_tbl+1)
	MAKE_LITERAL_PAIR (const_tbl+372, const_tbl+390)
	MAKE_LITERAL_PAIR (const_tbl+407, const_tbl+1)
	MAKE_LITERAL_PAIR (const_tbl+261, const_tbl+424)
	MAKE_LITERAL_PAIR (const_tbl+441, const_tbl+1)
	MAKE_LITERAL_PAIR (const_tbl+187, const_tbl+458)
	MAKE_LITERAL_PAIR (const_tbl+178, const_tbl+475)
	MAKE_LITERAL_PAIR (const_tbl+101, const_tbl+492)
	MAKE_LITERAL_INT (80)
fvar_tbl:
  dq T_UNDEFINED
	dq T_UNDEFINED
...
love:
    ...
    ret
		doneclousure1:
    mov qword [fvar_tbl + WORD_SIZE*53], rax;setting fvar:set-box!
    ...
    ret
		doneclousure2:
    mov qword [fvar_tbl + WORD_SIZE*52], rax;setting fvar:unbox
    mov rax, SOB_VOID_ADDRESS
		call write_sob_if_not_void
    ...
    ret
		doneclousure3:
    mov qword [fvar_tbl + WORD_SIZE*51], rax;setting fvar:box
    mov rax, SOB_VOID_ADDRESS
		call write_sob_if_not_void
    ...
    ret
		doneclousure4:
    mov qword [fvar_tbl + WORD_SIZE*50], rax;setting fvar:list
    mov rax, SOB_VOID_ADDRESS
		call write_sob_if_not_void
    ...
    push rax
    push qword 1
    mov rax, qword [fvar_tbl + WORD_SIZE*51];get fvar:box
		IS_UNDEF sym51
    violation_check rax, T_CLOSURE
      REST_APPLIC
    ...
    push SOB_NIL_ADDRESS
			mov rax, qword [fvar_tbl + WORD_SIZE*49];get fvar:append
		IS_UNDEF sym49
    push rax
			mov rax, qword [fvar_tbl + WORD_SIZE*50];get fvar:list
		IS_UNDEF sym50
    push rax
			mov rax, qword [fvar_tbl + WORD_SIZE*3];get fvar:cdr
		IS_UNDEF sym3
    ...
    violation_check rax, T_CLOSURE
      REST_APPLIC
    mov qword [fvar_tbl + WORD_SIZE*35], rax;setting fvar:reverse
    mov rax, SOB_VOID_ADDRESS
		call write_sob_if_not_void
    ...
    push rax
		...
		IS_UNDEF sym49
    push rax
			mov rax, qword [fvar_tbl + WORD_SIZE*35];get fvar:reverse
		IS_UNDEF sym35
    push rax
			mov rax, qword [fvar_tbl + WORD_SIZE*11];get fvar:null?
		IS_UNDEF sym11
    ...
    mov rax, qword [fvar_tbl + WORD_SIZE*47];get fvar:list?
		IS_UNDEF sym47
    violation_check rax, T_CLOSURE
    TP_SHIFT_FRAME 1
    REST_APPLICTP
    ...
    violation_check rax, T_CLOSURE
      REST_APPLIC
    mov qword [fvar_tbl + WORD_SIZE*41], rax;setting fvar:length
    mov rax, SOB_VOID_ADDRESS
		call write_sob_if_not_void

		
    push SOB_NIL_ADDRESS
			mov rax, qword [fvar_tbl + WORD_SIZE*41];get fvar:length
		IS_UNDEF sym41
    push rax
			mov rax, qword [fvar_tbl + WORD_SIZE*34];get fvar:=
		IS_UNDEF sym34
    ...
    violation_check rax, T_CLOSURE
      REST_APPLIC
    mov qword [fvar_tbl + WORD_SIZE*37], rax;setting fvar:not
    mov rax, SOB_VOID_ADDRESS
		call write_sob_if_not_void
    ...
    violation_check rax, T_CLOSURE
      REST_APPLIC
    mov qword [fvar_tbl + WORD_SIZE*46], rax;setting fvar:number?
    mov rax, SOB_VOID_ADDRESS
		call write_sob_if_not_void
    ...
    push rax
    push qword 2
    mov rax, qword [fvar_tbl + WORD_SIZE*45];get fvar:map
		IS_UNDEF sym45
    violation_check rax, T_CLOSURE
      REST_APPLIC
    push rax
    ...
    violation_check rax, T_CLOSURE
      REST_APPLIC
    mov qword [fvar_tbl + WORD_SIZE*45], rax;setting fvar:map
    mov rax, SOB_VOID_ADDRESS
		call write_sob_if_not_void
    ...
    violation_check rax, T_CLOSURE
      REST_APPLIC
    mov qword [fvar_tbl + WORD_SIZE*43], rax;setting fvar:list->vector
    mov rax, SOB_VOID_ADDRESS
		call write_sob_if_not_void
    ...
    violation_check rax, T_CLOSURE
      REST_APPLIC
    mov qword [fvar_tbl + WORD_SIZE*44], rax;setting fvar:vector->list
    mov rax, SOB_VOID_ADDRESS
		call write_sob_if_not_void

		
    push SOB_NIL_ADDRESS
			mov rax, qword [fvar_tbl + WORD_SIZE*43];get fvar:list->vector
		IS_UNDEF sym43
    push rax
    push qword 1
    EXTEND_VEC rax, rbx
    ...
    violation_check rax, T_CLOSURE
      REST_APPLIC
    mov qword [fvar_tbl + WORD_SIZE*42], rax;setting fvar:vector
    mov rax, SOB_VOID_ADDRESS
		call write_sob_if_not_void
    ...
    push rax
		...
		IS_UNDEF sym2
    push rax
			mov rax, qword [fvar_tbl + WORD_SIZE*37];get fvar:not
		IS_UNDEF sym37
    push rax
			mov rax, qword [fvar_tbl + WORD_SIZE*34];get fvar:=
		IS_UNDEF sym34
    ...
    violation_check rax, T_CLOSURE
      REST_APPLIC
    mov qword [fvar_tbl + WORD_SIZE*40], rax;setting fvar:>
    mov rax, SOB_VOID_ADDRESS
		call write_sob_if_not_void

		
    push SOB_NIL_ADDRESS
			mov rax, qword [fvar_tbl + WORD_SIZE*40];get fvar:>
		IS_UNDEF sym40
    push rax
    push qword 1
    EXTEND_VEC rax, rbx
    ...
    violation_check rax, T_CLOSURE
      REST_APPLIC
    mov qword [fvar_tbl + WORD_SIZE*39], rax;setting fvar:positive?
    mov rax, SOB_VOID_ADDRESS
		call write_sob_if_not_void
    ...
    violation_check rax, T_CLOSURE
      REST_APPLIC
    mov qword [fvar_tbl + WORD_SIZE*38], rax;setting fvar:negative?
    mov rax, SOB_VOID_ADDRESS
		call write_sob_if_not_void
    ...
    push rax
    push qword 2
    mov rax, qword [fvar_tbl + WORD_SIZE*36];get fvar:equal?
		IS_UNDEF sym36
    violation_check rax, T_CLOSURE
      REST_APPLIC
    cmp rax, SOB_FALSE_ADDRESS
    ...
    violation_check rax, T_CLOSURE
      REST_APPLIC
    mov qword [fvar_tbl + WORD_SIZE*36], rax;setting fvar:equal?
    mov rax, SOB_VOID_ADDRESS
		call write_sob_if_not_void
