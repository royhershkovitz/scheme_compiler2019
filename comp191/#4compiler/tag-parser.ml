...
(*let rec  string_eq st1 st2 i =
  let recstring_eq = string_eq st1 st2 in
  if(i >= String.length st1)
  then true
  else
  if(st1.[i] == st2.[i]) then recstring_eq (i+1) 
  else false;;

let string_eq st1 st2 = 
  if(String.length st1 == String.length st2)
  then string_eq st1 st2 0
  else false ;;*)
...
and parse_list_quasiquote sexpr = 
  ...
  | Pair(Symbol "unquote", Pair(sexpr, Nil)) -> sexpr
  | Pair(Symbol "quote", _) -> parse_single_quasiquote sexpr
...
let rec cumulative_tagger sexpr =           
     ...
     match sexpr with 
      ...
      (*| Pair (Symbol "quote", Pair(Symbol(st), Nil)) -> Const(Sexpr(Symbol("'"^st)))*)
      (*Const(Sexpr(Pair ((Symbol "quote"), Pair(Symbol(st), Nil))))*)
