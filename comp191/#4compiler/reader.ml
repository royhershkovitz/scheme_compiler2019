 ...
 let proper_list_of_exprs lst = 
  ...
  let pairs_three_dots ch1 nt ch2 dots_state str = 
    ...
    if(dots_state) then ( let (_, rest) = (disj (return_char_list (char ch2)) nt_epsilon) es in
      (sexpr, rest) )
    else  let (_, rest) = (disj (return_char_list (char ch2)) nt_epsilon) es in
    let (_, rest) = pack (caten star_space three_dots_nt) (fun(_)-> []) rest in 
    (sexpr, rest);;
...
(* READ SEXPR SECTION *)  
  ...
  let rec cumulative_parser is_three_dots string = 
      (*print_string (list_to_string string) ;
      print_string "\n" ;*)
      ...
      let vector_nt str = 
        let (_, s) = (char '#') str in 
        try pack (make_enclosed (char '(') star_space_comment (char ')'))
                  (fun (_) -> Vector([])) s
        with X_no_match ->
            pack (make_enclosed (char '(') (star cul_par) (char ')'))
                  (fun (lst) -> Vector(lst)) s in

      let list_nt s =         
        let cul_par = star cul_par in
        let cul_par_dot_mode = star cul_par_dot_mode in
        try pack (disj (make_enclosed (char '(') star_space_comment (char ')'))
                       (make_enclosed (char '[') star_space_comment (char ']')))
                  (fun (_) -> Nil) s
        with X_no_match ->
          pack (disj(disj(pairs_three_dots '(' cul_par_dot_mode ')' false) 
              ...
              (fun (lst) -> proper_list_of_exprs lst) s in

      let dotted_list_nt = 
        let cul_par = (caten (plus cul_par) (disj (pack (caten (char '.') cul_par) 
                                          (fun (_, sexpr) -> [sexpr])) nt_epsilon)) in
        let cul_par_dot_mode = (caten (plus cul_par_dot_mode) 
        (disj (pack (caten (char '.') cul_par_dot_mode)
            (fun (_, sexpr) -> [sexpr])) nt_epsilon)) in
          pack (disj (disj (pairs_three_dots '(' cul_par_dot_mode ')' false) 
                           (pairs_three_dots '[' cul_par_dot_mode ']' false))
                    ...
                    (disj (make_enclosed (char '(') cul_par (char ')'))
                          ...
                          (fun (lst, expr) -> 
                          if(expr = [])
                          then proper_list_of_exprs (lst)
                          else improper_list_of_exprs (lst@expr)) in
