...
let  grn =  "\027[38;5;052m"
let yel  = "\027[38;5;26m"
...
let test = fun test_number equal_test -> 
    if equal_test 
        then 
            ...
            (if !failure_info = "as not as expected" then failure_info := Printf.sprintf "with got\n%s\nwhile expected\n%s" !got !expected);
...
let test_Cond_Macro_Expansion = fun () ->
    ...
    test 32 (expr_eq (execute_tag_parse_expression (
    Pair(Symbol "cond", 
        Pair(Pair(Symbol "f", Pair(Symbol "=>", Pair(Symbol "g", Nil))),
        Pair(Pair(Symbol "f", Pair(Pair(Symbol "lambda", Pair(Symbol "s", Pair(Symbol "x", Nil))),Nil)), 
    ...
    [Var("f");LambdaSimple([],Var("g")); LambdaSimple([],If(Var("f"),LambdaOpt([],"s",Var("x")),Applic(LambdaSimple(["value"; "f"],If(Var("value"),Applic(Applic(Var("f"),[]),[Var("value")]),Const(Void))),[Var("f");LambdaSimple([],Var("g"))])))]))));                          
