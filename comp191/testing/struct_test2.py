...
let input = ({});; \
let output = ({});; \
Printf.printf "%b" (List.for_all2 expr_eq (Tag_Parser.tag_parse_expressions input) output);;'
...
def test():
    ...
    for in_file_path in sorted([f for f in os.listdir(".") if f.endswith(".in")], key=lambda f: int(f[:f.index(".")])):
        ...
        try:
            proc = subprocess.run(["ocaml", "-stdin"], input=test_string, encoding="ascii", stdout=PIPE, stderr=PIPE, timeout=timeout)
